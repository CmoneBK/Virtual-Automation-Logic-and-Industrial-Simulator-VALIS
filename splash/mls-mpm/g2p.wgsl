...
fn g2p(@builtin(global_invocation_id) id: vec3<u32>) {
    if (id.x < numParticles) {
        ...
        // Solid boundary correction: push particle to exit face in direction of motion.
        // Both axes handled: vertical (fell/rose) and horizontal (moved sideways into wall).
        // Horizontal correction pushes in the direction of travel so particles flow off ledges.
        let solidX = u32(clamp(i32(floor(particles[id.x].position.x)), 0, 203));
        let solidY = u32(clamp(i32(floor(particles[id.x].position.y)), 0, 153));
        let sv = particles[id.x].v;
        if (textureLoad(solidTex, vec2u(solidX, solidY), 0).r != 0u) {
            if (abs(sv.y) >= abs(sv.x)) {
                // Vertical exit: fell from above or rose from below
                if (sv.y <= 0.) {
                    particles[id.x].position.y = f32(solidY) + 1.001; // fell → sit on top face
                    particles[id.x].v.y = max(sv.y, 0.);
                } else {
                    particles[id.x].position.y = f32(solidY) - 0.001; // rose → push to bottom face
                    particles[id.x].v.y = min(sv.y, 0.);
                }
            } else {
                // Horizontal exit: push to exit face in direction of travel
                if (sv.x > 0.) {
                    particles[id.x].position.x = f32(solidX) + 1.001; // moving right → exit right face
                    particles[id.x].v.x = max(sv.x, 0.);
                } else {
                    particles[id.x].position.x = f32(solidX) - 0.001; // moving left → exit left face
                    particles[id.x].v.x = min(sv.x, 0.);
                }
            }
        }
