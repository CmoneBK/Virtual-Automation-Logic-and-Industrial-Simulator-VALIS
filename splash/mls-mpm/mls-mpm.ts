...
import drainParticlesShader from './drainParticles.wgsl'
...
export class MLSMPMSimulator {
    ...
    drainPipeline: GPUComputePipeline
    ...
    drainBindGroup: GPUBindGroup

    particleBuffer: GPUBuffer
    dtBuffer: GPUBuffer
    densityGridBuffer: GPUBuffer

    // Drain (position-based stream compaction)
    drainZonesBuffer: GPUBuffer
    drainZonesCountBuffer: GPUBuffer
    drainCounterBuffer: GPUBuffer
    drainOutputBuffer: GPUBuffer
    drainStagingBuffer: GPUBuffer
    _drainZoneQueue: number[] = []

    device: GPUDevice

    solidTexture: GPUTexture
    solidTextureSampler: GPUSampler
        ...
        ) 
    {
        ...
        this.drainPipeline = device.createComputePipeline({
            label: 'drain particles pipeline',
            layout: 'auto',
            compute: { module: device.createShaderModule({ code: drainParticlesShader }) },
        });
        ...
        const MAX_DRAIN_ZONES = 32;
        this.drainZonesBuffer = device.createBuffer({
            label: 'drain zones buffer',
            size: 16 * MAX_DRAIN_ZONES, // vec4f per zone
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
        });
        this.drainZonesCountBuffer = device.createBuffer({
            label: 'drain zones count buffer',
            size: 4,
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
        });
        this.drainCounterBuffer = device.createBuffer({
            label: 'drain counter buffer',
            size: 4,
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST,
        });
        this.drainOutputBuffer = device.createBuffer({
            label: 'drain output buffer',
            size: mlsmpmParticleStructSize * maxParticleCount,
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC,
        });
        this.drainStagingBuffer = device.createBuffer({
            label: 'drain staging buffer',
            size: 4,
        ...
        // VALIS solid grid texture (204×154, matches Splash grid 1:1, r8uint, initially all zeros)
        this.solidTexture = device.createTexture({
            label: 'valis solid grid texture',
            size: [204, 154],
            format: 'r8uint',
            usage: GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_DST,
        });
        const emptySolid = new Uint8Array(204 * 154);
        ...
        this.updateGridBindGroup = device.createBindGroup({
            ...
            entries: [
                ...
                { binding: 7, resource: this.solidTexture.createView() },
            ],
        })
        this.g2pBindGroup = device.createBindGroup({
            ...
            entries: [
                ...
                { binding: 5, resource: { buffer: this.dtBuffer }},
                { binding: 6, resource: this.solidTexture.createView() },
            ],
        })
        ...
        this.drainBindGroup = device.createBindGroup({
            layout: this.drainPipeline.getBindGroupLayout(0),
            entries: [
                { binding: 0, resource: { buffer: particleBuffer } },
                { binding: 1, resource: { buffer: this.drainOutputBuffer } },
                { binding: 2, resource: { buffer: this.drainCounterBuffer } },
                { binding: 3, resource: { buffer: this.numParticlesBuffer } },
                { binding: 4, resource: { buffer: this.drainZonesBuffer } },
                { binding: 5, resource: { buffer: this.drainZonesCountBuffer } },
            ],
        });
    ...
    // Upload a new solid grid from VALIS (Uint8Array of 204×154 bytes, row-major)
    ...
    // Queue a drain zone (world-space rect: x, y, width, height).
    // Zones are consumed once per frame by runDrain().
    queueDrainZone(x: number, y: number, w: number, h: number) {
        this._drainZoneQueue.push(x, y, w, h);
    }

    // GPU stream-compaction: remove particles in queued drain zones.
    // Call after physics substeps, before rendering.
    runDrain(commandEncoder: GPUCommandEncoder) {
        const numZones = this._drainZoneQueue.length / 4;
        if (numZones === 0 || this.numParticles === 0) {
            this._drainZoneQueue.length = 0;
            return;
        }

        // Upload drain parameters
        this.device.queue.writeBuffer(this.drainCounterBuffer, 0, new Uint32Array([0]));
        this.device.queue.writeBuffer(this.drainZonesBuffer, 0, new Float32Array(this._drainZoneQueue));
        this.device.queue.writeBuffer(this.drainZonesCountBuffer, 0, new Uint32Array([numZones]));
        this._drainZoneQueue.length = 0;

        // Compaction pass
        const pass = commandEncoder.beginComputePass({ label: 'drain pass' });
        pass.setBindGroup(0, this.drainBindGroup);
        pass.setPipeline(this.drainPipeline);
        pass.dispatchWorkgroups(Math.ceil(this.numParticles / 64));
        pass.end();

        // Copy compacted particles back to particleBuffer
        commandEncoder.copyBufferToBuffer(
            this.drainOutputBuffer, 0,
            this.particleBuffer, 0,
            mlsmpmParticleStructSize * this.maxParticleCount
        );
        // Update GPU numParticles from atomic counter
        commandEncoder.copyBufferToBuffer(this.drainCounterBuffer, 0, this.numParticlesBuffer, 0, 4);
        // Copy counter to staging for async JS readback
        commandEncoder.copyBufferToBuffer(this.drainCounterBuffer, 0, this.drainStagingBuffer, 0, 4);
    }

    // Run copyPosition pass to refresh posvelBuffer from particleBuffer.
    // Call after runDrain() so the renderer sees the compacted positions.
    runCopyPosition(commandEncoder: GPUCommandEncoder) {
        const pass = commandEncoder.beginComputePass({ label: 'post-drain copyPosition' });
    ...
    // Schedule async readback of particle count after GPU submit.
    scheduleCounterReadback() {
        if (this.drainStagingBuffer.mapState !== 'unmapped') return;
        this.drainStagingBuffer.mapAsync(GPUMapMode.READ).then(() => {
            const count = new Uint32Array(this.drainStagingBuffer.getMappedRange())[0];
            this.drainStagingBuffer.unmap();
            this.numParticles = count;
        }).catch(() => { /* device lost or destroyed */ });
    }

    // Add particles at normalized position (nx,ny in 0..1) with normalized radius
    // initBoxSize is the current simulation box [x, y, z]
    addParticles(nx: number, ny: number, nRadius: number, count: number, initBoxSize: number[]) {
        if (this.numParticles + count > this.maxParticleCount) {
            count = this.maxParticleCount - this.numParticles;
        }
        if (count <= 0) return;

        // Map 0..1 into the active simulation zone (MARGIN..BOX-MARGIN) so canvas edges align
        const MARGIN = 2;
        const cx = MARGIN + nx * (initBoxSize[0] - 2 * MARGIN);
        const cy = MARGIN + (1 - ny) * (initBoxSize[1] - 2 * MARGIN); // flip Y: VALIS Y-down → Splash Y-up
        const cz = initBoxSize[2] / 2;        // center in Z
        const r = nRadius * initBoxSize[0];

        const buf = new ArrayBuffer(mlsmpmParticleStructSize * count);
        let added = 0;
        for (let attempt = 0; added < count && attempt < count * 4; attempt++) {
            const dx = (Math.random() * 2 - 1) * r;
            const dy = (Math.random() * 2 - 1) * r;
            const dz = (Math.random() * 2 - 1) * r * 0.5;
            if (dx * dx + dy * dy > r * r) continue;
            const offset = mlsmpmParticleStructSize * added;
            const posView = new Float32Array(buf, offset, 3);
            posView.set([
                Math.max(2, Math.min(initBoxSize[0] - 2, cx + dx)),
                Math.max(2, Math.min(initBoxSize[1] - 2, cy + dy)),
                Math.max(1, Math.min(initBoxSize[2] - 1, cz + dz)),
            ]);
            added++;
        }
        const writeOffset = mlsmpmParticleStructSize * this.numParticles;
        this.device.queue.writeBuffer(this.particleBuffer, writeOffset, buf, 0, mlsmpmParticleStructSize * added);
        this.changeNumParticles(this.numParticles + added);
    }
}